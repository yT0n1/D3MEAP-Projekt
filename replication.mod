...
param Fragment_Size {i in 1..Fragments} := Uniform(0,100);						
param Num_Queries := 9; #nope menge an columns
param Query_Frequency {i in 1..Query} := Uniform(0,100);
param Query_Cost {i in 1..Query} := Uniform(0,100);
param Workload {i in 1..Query} := Query_Cost[i] *  Query_Frequency[i];
param Num_Nodes := 4;
